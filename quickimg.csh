...
#image stokes i
invert vis=${in} options=mfs,double map=${outroot}-${stokes}.mp beam=${outroot}-${stokes}.bm select=$select #cell=${cs} imsize=${ims},${ims} stokes=$stokes
clean map=${outroot}-${stokes}.mp beam=${outroot}-${stokes}.bm out=${outroot}-${stokes}.cl niters=1000 cutoff=0.002 #region='relpixel,boxes('${cleanreg}')'
