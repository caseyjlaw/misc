...
#    ims,snr,candints = qimg.imgallfullfilterxy(n.outer(u[d['iterint']/2], d['freq']/d['freq_orig'][0]), n.outer(v[d['iterint']/2], d['freq']/d['freq_orig'][0]), data[dmind], d['sizex'], d['sizey'], d['res'], d['sigma_image'])
...
#            print i, candints[i], minint, maxint, peakl, peakm, snipint, data0_snip.mean()
            ...
            if sm < n.sqrt(d['nchan']/snr[i]**2 + d['specmodfilter']):
                print 'Got one!  Int=%d, DM=%d, SNR_im=%.1f @ (%d,%d). SNR_lc=%.1f., SM=%.1f < %.1f, so keeping candidate.' % (d['nskip']+d['itercount']+candints[i], d['dmarr'][dmind], snr[i], peakl, peakm, snrlc, sm, n.sqrt(d['nchan']/snr[i]**2 + d['specmodfilter']))
