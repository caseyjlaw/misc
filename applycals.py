...
class solutions():
    ...
    def __init__(self, gainfile, fluxfile='', flagants=True):
        ...
        # if gaincal file with flux scale provided, scale self.gain to it (per spw,pol)
        if fluxfile:
            self.parseflux(fluxfile)
...
#        print 'Parsed bp table solutions for %d solutions, %d ants, %d spw, and %d pols' % (nUniqueTimesBP, nants, nSpws, nPolarizations)
    ...
    def setselection(self, time, freqs, spws=[0,1], pols=['RR','LL'], verbose=0):
    ...
    def calc_flag(self, antnums, sig=3.0):
        """ Calculates antennas to flag, based on bad gain and bp solutions.
        Needs antnum array to label antennas for printing.
        """
...
#        badgain = n.where(gamp < gamp.mean() - sig*gamp.std())
        ...
        print 'Flagging low/bad gains for ant/spw/pol:', antnums[badgain[0]], badgain[1], badgain[2]
    ...
    def apply(self, data, blarr):
        ...
        # flag bad ants
        if self.flagants:
            badants = self.calc_flag(n.unique(blarr))
        else:
            badants = n.array([[]])
