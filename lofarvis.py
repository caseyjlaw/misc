...
"""lofarvis.py - visualization of lofar visibilities
modified from evlavis.py
claw, 9 april 2012
"""

import string, os, aipy
...
#        Q = sigt0toQ(dibimeanstd, self.nants)              # alternate for Q
    ...
    def phaseshift(self, l, m):
        ...
        Returns new data array. Overwrite the existing data and dataph array, if you'd like to treat this as the new data.
    ...
    def imagetrack(self, tbin, bgwindow=0, size=48000, res=500, clean=True):
        tr = self.tracksub(tbin, bgwindow=bgwindow)[0].mean(axis=1)
...
def pulse_search_bisp(fileroot, pathin, pathout, nints=600, edge=15):
    ...
    If lofar data files have only one integration, this needs to be modified a bit.
    """
    ...
    for file in filelist:
        for nskip in range(startint, maxints-(nints-edge), nints-edge):
            ...
            lofar = lofar(pathin + file, nints=nints, nskip=nskip, selectpol=['XX'])
            ...
            lofar.spec(n.unique(indagg), save=1, pathout=pathout)
