...
"""mwavis.py - visualization of mwa visibilities
...
def pulse_search_bisp(fileroot, pathin, pathout, nints=600, edge=15):
    ...
    If mwa data files have only one integration, this needs to be modified a bit.
    """
    ...
    for file in filelist:
        for nskip in range(startint, maxints-(nints-edge), nints-edge):
            ...
            mwa = mwa(pathin + file, nints=nints, nskip=nskip, selectpol=['XX'])
            ...
            mwa.spec(n.unique(indagg), save=1, pathout=pathout)
