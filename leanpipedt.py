...
def estimate_noiseperbl(data0):
    ...
    logging.info('Sigma clip of %.3f to %.3f keeps %d%% of data' % (data0meanmin, data0meanmax, (100.*len(good[0]))/len(data0mean.flatten())))
    logging.info('Estimate of noise per baseline: %.3f' % noiseperbl)
    return noiseperbl
...
#            proplist.append( (snr, img, specpol) )     # full set
        ...
        if verbose:
            logging.debug(loclist, proplist)
        ...
        if verbose:
            logging.debug('No cands to save...')
...
#                maxint = min(candints[i]+twindow, len(data0))
                data0_snip = n.array(data0.data[candints[i]].copy())         # get candidate integration
...
#                snrlc = bflc[snipint] / bflc[range(0,snipint-1)+range(snipint+2,twindow)].std()    # lc snr of event. more accurate than img snr
                ...
                if sm < n.sqrt(d['nchan']/snr[i]**2 + d['specmodfilter']):
                    logging.info('Got one!  Int=%d, DM=%d, dt=%d, SNR_im=%.1f @ (%d,%d), SM=%.1f < %.1f, so keeping candidate.' % (d['nskip']+d['itercount']+candints[i]*d['dtarr'][dtind], d['dmarr'][dmind], d['dtarr'][dtind], snr[i], peakl, peakm, sm, n.sqrt(d['nchan']/snr[i]**2 + d['specmodfilter'])))
#                spectra.append(data0_snip.mean(axis=1))         # get real part of spectrogram of candidate
                    goodcandints.append(i)
                    lmarr.append( (l1, m1) )
                else:
                    logging.info('Almost... Int=%d, DM=%d, dt=%d, SNR_im=%.1f @ (%d,%d), SM=%.1f > %.1f, so rejecting candidate.' % (d['nskip']+d['itercount']+candints[i]*d['dtarr'][dtind], d['dmarr'][dmind], d['dtarr'][dtind], snr[i], peakl, peakm, sm, n.sqrt(d['nchan']/snr[i]**2 + d['specmodfilter'])))
                ...
                if snr2 > d['sigma_image']:
                    logging.info('Got one!  Int=%d, DM=%d, dt=%d: SNR_im=%.1f @ (%.2e,%.2e) and SNR2=%.1f @ (%.2e, %.2e), so keeping candidate.' % (d['nskip']+d['itercount']+candints[i]*d['dtarr'][dtind], d['dmarr'][dmind], d['dtarr'][dtind], snr[i], l1, m1, snr2, l2, m2))
                    goodcandints.append(i)
                    lmarr.append( (l1, m1) )
                    lm2arr.append( (l2, m2) )
                    snr2arr.append(snr2)
                else:
                    logging.info('Almost...  Int=%d, DM=%d, dt=%d: SNR_im=%.1f @ (%.2e,%.2e) and SNR2=%.1f @ (%.2e, %.2e), so rejecting candidate.' % (d['nskip']+d['itercount']+candints[i]*d['dtarr'][dtind], d['dmarr'][dmind], d['dtarr'][dtind], snr[i], l1, m1, snr2, l2, m2))
...
def readprep(d):
    ...
    if os.path.exists(pklname):
        ...
        logging.info('Initializing integration time (s): %.3f' % d['inttime'])
...
#        d['npol'] = len(selectpol)
        ...
        logging.info('Initializing %d polarizations' % (d['npol']))
    ...
    if len(d['excludeants']):
        logging.info('Excluding ant(s) %s' % str(d['excludeants']))
    ...
    logging.info('Initializing nants: %d' % d['nants'])
    logging.info('Initializing nbl: %d' % d['nbl'])
    ...
    d['nchan'] = len(d['chans'])
    logging.info('Initializing nchan: %d' % d['nchan'])
    ...
    logging.info('First integration of scan: %s' % str(qa.time(qa.quantity(starttime_mjd,'d'),form=['ymd'],prec=9)[0]))
    logging.info('Reading scan %s for times %s to %s' % (str(scanlist[scan]), str(qa.time(qa.quantity(starttime_mjd+timeskip/(24.*60*60),'d'),form=['hms'], prec=9)[0]), str(qa.time(qa.quantity(starttime_mjd+(timeskip+(nints+1)*d['inttime'])/(24.*60*60), 'd'), form=['hms'], prec=9)[0])))
    ...
    logging.info('Ideal uvgrid size=(%d,%d) for res=%d' % (d['full_sizex'], d['full_sizey'], d['res']))

    if d['size'] == 0:
        d['sizex'] = d['full_sizex']
        d['sizey'] = d['full_sizey']
        logging.info('Using uvgrid size=(%d,%d) (2**(%d,%d)*3**(%d,%d) = (%d,%d)) and res=%d' % (d['sizex'], d['sizey'], p2x, p2y, p3x, p3y, 2**p2x*3**p3x, 2**p2y*3**p3y, d['res']))
    else:
        d['sizex'] = d['size']
        d['sizey'] = d['size']
        logging.info('Using uvgrid size=(%d,%d) and res=%d' % (d['sizex'], d['sizey'], d['res']))

    d['size'] = max(d['sizex'], d['sizey'])
    logging.info('Image memory usage for %d threads is %d GB' % (d['nthreads'], 8 * d['sizex']/d['res'] * d['sizey']/d['res'] * d['iterint'] * d['nthreads']/1024**3))
    ...
    if d['telcalfile']:    # apply telcal solutions
        ...
        for i in range(len(d['selectpol'])):
            try:
                ...
                logging.info('Applied cal for spw %d and pol %s' % (spwbin, str(d['selectpol'][i])))
            except:
                pass
            ...
            if d['telcalfile']:    # apply telcal solutions
                ...
                for i in range(len(d['selectpol'])):
                    try:
                        ...
                        logging.info('Applied cal for spw %d and pol %s' % (spw, d['selectpol'][i]))
                    except:
                        pass
...
#    data1_mem = mp.sharedctypes.RawArray(ctypes.c_float, (d['iterint']*d['nbl']*d['nchan']*d['npol']*2))    # x2 to store complex values in single array
    ...
    logging.info('Read first iteration with shape %s' % str(datacap.shape))

    while 1:
#            name = mp.current_process().name
#            print '%s: filling buffer' % name
        datanext, unext, vnext, wnext, timenext = readiter(d)
        logging.info('Read next %d ints from iter %d' % (len(datanext), d['itercount1']+iterint))
        ...
        if ((len(datanext) < iterint) and d['iterstatus1']):  # read once more if data buffer is too small. don't read if no data! iterator gets confused.
            ...
            logging.info('Read another %d ints for iter %d' % (len(datanext2), d['itercount1']+iterint))
...
# flag data before moving into place
            ...
            if (d['flagmode'] == 'standard'):
                ...
                logging.info('No real-time flagging.')

            if d['filtershape'] == 'z':
                logging.info('Subtracting mean visibility in time...')
            ...
            if d['savecands'] and n.any(datacal[d['iterint']/2]):
                ...
                noisefile = d['workdir'] + 'noise_' + d['filename'].rstrip('.ms') + '.pkl'
            ...
            logging.info('Ready to move data into place for itercount %d' % d['itercount1'])
            emove.wait()
            emove.clear()
            if 'lowmem' in d['searchtype']:
                data[...,0] = datacal[...,0]
...
#            flag[:] = flag1[:]
        ...
        # NOW MAKE SURE ALL ENDS GRACEFULLY
            ...
            logging.info('End of data (in buffer)')
        ...
        if not d['iterstatus']:          # using iterstatus1 is more conservative here. trying to get around hangup on darwin.
            logging.info('End of data (iterator)')
            eproc.set()
            ms.iterend()
            ms.close()
            break
...
def readtriggerloop(d, eproc, emove):
    ...
    while 1:
        eproc.wait()
        eproc.clear()
        logging.info('Iterating readloop...')
        emove.set()
...
def processloop(d, eproc, emove):
    ...
    while 1:
        eproc.wait()
        eproc.clear()
        logging.info('Processing for itercount %d. ' % (d['itercount']))
...
#        lib.dataflag(datacal, sigma=1000., mode='blstd')
        ...
        if n.any(data):
            for dmind in xrange(len(d['dmarr'])):
                logging.info('Processing DM = %d (max %d)' % (d['dmarr'][dmind], d['dmarr'][-1]))
                for dtind in xrange(len(d['dtarr'])):
                    result = pool.apply_async(imgallloop, [d, dmind, dtind, beamnum])
                    resultlist.append(result)
        else:
            logging.info('Data for processing is zeros. Moving on...')
        ...
        logging.info('Adding %d from itercount %d of %s. ' % (len(candslist), d['itercount'], d['filename']))
        ...
        if not d['iterstatus']:
            ...
            logging.info('End of processloop')
            break
...
def pipe_thread(filename, nints=200, nskip=0, iterint=200, spw=[0], chans=range(5,59), dmarr=[0.], dtarr=[1], fwhmsurvey=0.5, fwhmfield=0.5, selectpol=['RR','LL'], scan=0, datacol='data', size=0, res=0, sigma_bisp=6.5, sigma_image=6.5, filtershape=None, secondaryfilter='fullim', specmodfilter=1.5, searchtype='imageall', telcalfile='', telcalcalibrator='', gainfile='', bpfile='', savecands=0, candsfile='', flagmode='standard', flagantsol=True, nthreads=1, wplanes=0, excludeants=[]):
    ...
    if 'lowmem' in searchtype:
        logging.info('Running in \'lowmem\' mode. Reading pols %s, then summing after cal, flag, and filter. Flux scale not right if pols asymmetrically flagged.' % selectpol)
        d['npol'] = 1
    else:
        d['npol'] = len(selectpol)
    ...
    # define filename and workdir to allow full path definitions (and using rq)
    ...
    logging.info('Preparing to read...')
...
#    d['datadelay'] = n.array([[lib.calc_delay(d['freq'], d['inttime']*d['dtarr'][i], d['dmarr'][j]).max() for i in range(len(d['dtarr']))] for j in range(len(d['dmarr']))])  # keep track of delay shift as array indexed with [dmind][dtind]
    ...
    d['starttime'] = tt
    logging.info('Start time: %s_%s_%s:%s:%s:%s' % (tt.tm_year, tt.tm_mon, tt.tm_mday, tt.tm_hour, tt.tm_min, tt.tm_sec))
    # define candidate file
    if d['savecands']:
        if not d['candsfile']:
            timestring = '%s_%s_%s:%s:%s:%s' % (tt.tm_year, tt.tm_mon, tt.tm_mday, tt.tm_hour, tt.tm_min, tt.tm_sec)
            d['candsfile'] = d['workdir'] + 'cands_' + d['filename'].rstrip('.ms')+'.pkl'
    ...
    logging.info('Preparing shared memory objects...')
#    data_mem = {}
#    data = {}
#    for dmind in xrange(len(d['dmarr'])):
...
#        data[dmind][:] = n.zeros(((d['iterint']/d['resamplearr'][dmind]), d['nbl'], d['nchan'], d['npol']))
    ...
    logging.info('Visibility memory usage is %d GB' % (8*(totalnint * d['nbl'] * d['nchan'] * d['npol'])/1024**3))  # factor of 2?
...
#    flag = numpyview(flag_mem, 'bool', (iterint, d['nbl'], d['nchan'], d['npol']))
    ...
    logging.info('Starting processing and reading loops...')
    try:
        if searchtype:
            ...
            logging.info('Not starting read and process threads...')
    except KeyboardInterrupt:
        logging.info('Ctrl-C received. Shutting down threads...')
        pread.terminate()
        pproc.terminate()
        pread.join()
        pproc.join()
